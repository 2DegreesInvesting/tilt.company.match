...
```{r}
# Matched companies based on suggested matches
matched_rows <- to_edit %>% filter(suggest_match) %>% nrow()

# Share of matched companies from total loanbook companies
matched_share <- to_edit %>% filter(suggest_match) %>% nrow()/ loanbook %>% distinct(company_name) %>% nrow()
matched_share
...
misc_share <- loanbook %>%
  ...
  left_join(to_edit %>%
    ...
    count(misc_info), by = c("misc_info"), suffix = c("_loanbook", "_merged")) %>%
  mutate(n_share = n_merged / n_loanbook)

# Calculate loan exposure categorised based on misc_info
# id_tilt column is used as the substitute due to unavailability of loan amount column in sample data. Please replace id_tilt column with the loan amount column.

# Calculate number of loans and their share from total after grouping by misc_info
exposure_count <- to_edit %>%
  filter(suggest_match) %>%
  group_by(misc_info) %>%
  summarise(num = n()) %>%
  mutate(count_share = num/sum(num))

# Calculate sum of loan values and their share from total value after grouping by misc_info
exposure_sum <- to_edit %>%
  filter(suggest_match) %>%
  group_by(misc_info) %>%
  summarise(loan_exposure = sum(id_tilt)) %>%
  mutate(sum_share = loan_exposure/sum(loan_exposure))
```
