...
matched <- nrow(to_edit %>% dplyr::filter(suggest_match == TRUE))

# share of matched companies from total loanbook companies
matched_share <- nrow(to_edit %>% dplyr::filter(suggest_match == TRUE)) / nrow(loanbook %>% distinct(company_name))
...
share_df <- (loanbook %>% dplyr::count(misc_info)) %>%
  dplyr::left_join(to_edit %>%
    dplyr::filter(suggest_match == TRUE) %>%
    dplyr::count(misc_info), by = c("misc_info"), suffix = c("_loanbook", "_merged"))
share_df$share_misc <- share_df$n_merged / share_df$n_loanbook

# Guidance on loan exposure
# misc_info and id_tilt should be replaced based on target column names
exposure <- to_edit %>%
  dplyr::filter(suggest_match == TRUE) %>%
  dplyr::group_by(misc_info) %>%
  dplyr::summarise(sum_exposure = sum(id_tilt))
```
