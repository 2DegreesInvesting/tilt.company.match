...
#' ## tl;dr
#' 
#' > I just want to download the source code of this file so I can run it locally.
#' 
#' * [Find it on
#' GitHub](https://github.com/search?q=repo%3A2degreesinvesting%2Ftilt.company.match+filename%3A*.Rmd&type=code)
#' * Click the "Raw" tab.
#' * Save it to your computer, typically with right-click or Control+S.
#' 
#' > I just want to copy-paste and run all the code.
#' 
#' Find the code-only version of this file
#' [here](https://2degreesinvesting.github.io/tilt.company.match/articles/). The
#' text is still there but it's commented out.
#' 
#' ## Considerations
#' 
...
loanbook <- read_csv(loanbook_csv, show_col_types = FALSE)
loanbook
...
tilt <- read_csv(tilt_csv, show_col_types = FALSE)
tilt
...
#' write_csv(to_edit, "to_edit.csv", show_col_types = FALSE)
#' 
...
edited <- read_csv(edited_csv, show_col_types = FALSE)
edited
...
#' ```r
#' # You can install the readr package with: `install.packages("readr")`
#' final %>% write_csv("final.csv", show_col_types = FALSE)
#' ```
#' 
## ----echo=FALSE---------------------------------------------------------------
extdata_path <- here::here("inst", "extdata")
if (fs::dir_exists(extdata_path)) {
  knitr::purl(
    here::here("vignettes", "articles", "tilt-company-match.Rmd"),
    output = file.path(extdata_path, "tilt-company-match_code.R"),
    documentation = 2
  )
}
