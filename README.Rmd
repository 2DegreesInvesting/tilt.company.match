...
For cases for which no postcode, but country is available we match only on
country. For companies for which neither postcode nor country are around we
consider all companies potential candidates. We treat cases that have postcode
but not country the same way, it would possible induce false positive to match
via postcode over different countries.
