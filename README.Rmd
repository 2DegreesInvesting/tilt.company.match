...
Now let us insert duplicated matches for the company with the id 1.

```{r}
duplicate_in_loanbook <- manually_matched %>% 
  dplyr::mutate(accept_match = dplyr::if_else(id == 1, TRUE, accept_match))

knitr::kable(duplicate_in_loanbook %>% dplyr::filter(id == 1))
```
