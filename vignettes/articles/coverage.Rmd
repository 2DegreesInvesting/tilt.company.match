...
classifier to calculate the coverage based on that classifier. To calculate the
`misc_share` values, we need the `misc_info` column in both the tables.
Therefore, it is recommended to check whether the `misc_info` is present in both
of them.
We can also check the count of companies in the desired classification through
`x_misc` or `y_misc`.
...
misc_share <-
  left_join(x_misc, y_misc, by = c("misc_info"), suffix = c("_loanbook", "_merged")) %>%
