...
The goal of tilt.company.match is to help you match companies in a bank's
loanbook to companies in a dataset from the tilt-project.
...
Say you want to match a company in a bank's loanbook to a company in the tilt
...
For each of the two datasets -- the loanbook and tilt dataset -- the code here
expects these properties:

- The dataset is an R dataframe.
- The dataset has the columns `id`, `company_name`, `postcode`, and `country`.
- The dataset may or may not have other columns.
- The column `id` holds unique row-identifiers.

## Example

We'll use general purpose tools from
packages in the [tidyverse](https://www.tidyverse.org/), and more specific tools
from the packages [stringdist](https://CRAN.R-project.org/package=stringdist)
and
...
Here we'll use a "demo" loanbook. You should replace with your real loanbook.

```{r}
# TODO: Replace with your real loanbook and tilt datasets
...
**Expected columns:** Do you have the expected columns `id`, `company_name`, `postcode`, and `country`?
...
**Duplicates:** Do you have duplicates in `company_name`, `postcode` or `country`?

It's best if there is no duplicate in the columns `company_name`, `postcode` or
`country`. But if they have duplicates and they belong to different companies,
then you don't have to fix them.

```{r}
best_without_duplicates <- c("company_name", "postcode", "country")
report_duplicates(loanbook, best_without_duplicates)

# `misc_info` suggest the duplicates belong to different companies, so we're ok
loanbook %>% 
  filter(company_name == "Peasant Peter") %>% 
  filter(postcode == "01234")
```

**Missing values:** Do you have missing values (`NA`s) in non-nullable columns?
...
# A non-nullable columns has NA's. 
...
Use `to_alias()` to reduce the change you'll miss a match because of spurious
differences in the company name between the loanbook and tilt dataset. It helps
you get a less noisy, more consistent version of `company_name` in each of the
two datasets before matching them.
...
```{r}
reliable_shared_columns <- c("country", "postcode")
# WARNING: If you lack `postcode` use this instead:
# reliable_shared_columns <- "country"

candidates <- left_join(
    loanbook_alias, tilt_alias, 
    by = reliable_shared_columns,
    # Add the suffix "_tilt" to each column from the tilt dataset
    suffix = c("", "_tilt"),
    # Allow matching one to many
    multiple = "all"
)

# Example: 1 company in the loanbook matches 3 candidates in the tilt dataset
candidates %>% 
  filter(id == 1) %>%
  select(country, postcode, id, company_alias, id_tilt, company_alias_tilt)
```
...
```{r}
candidates_similarity <- candidates %>%
...
The values of `suggest_match` are either `TRUE` or `FALSE`. By default they are
set to `FALSE`, then set to `TRUE` where the value of `similarity` meets all of
these conditions:

- It's the highest among all other candidates.
- It's above a given threshold.
- It's the only such highest value in the group defined by a combination of `company_name` x `id` -- to avoid duplicates.

```{r}
# Decided upon extensive experience with r2dii.match function and processes
threshold <- 0.9

candidates_suggest_match <- candidates_similarity %>%
  # - It's the highest among all other candidates.
  group_by(id) %>%
  ...
  # `company_name` x `id` -- to avoid duplicates.
  mutate(duplicates = any(duplicated(company_name, postcode))) %>%
  filter(duplicates == FALSE) %>%
  select(id, id_tilt) %>%
  mutate(suggest_match = TRUE)

to_edit <- candidates_similarity %>%
...
editor. For example, you may write it as a .csv or .xlsx then open it in Excel
or GoogleSheets.
...
### Checking the dataset you edited

Manual work is prone to errors. Here you'll explore the edited dataset to ensure
it's correct.

- Ensure your R session is fresh (restart R if necessary).
- Read the "edited.csv" file back into R. Here we'll use a "demo" dataset but you
should use the real file you edited.
- Re-read your loanbook.

```{r}
# TODO: Replace with the file you really edited
# edited <- readr::read_csv("path/to/the/file/you/really/edited.csv")
edited <- demo_matched

# TODO: Replace with your real loanbook
# loanbook <- readr::read_csv("path/to/your/real/loanbook.csv")
loanbook <- demo_loanbook
```

- Use `report_no_matches()` to explore companies in the loanbook without a match
in the tilt dataset.
