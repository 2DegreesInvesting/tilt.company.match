...
If you lack the `postcode` column you can create one with "fake" values and move
on. This will add more manual-validation work.

```{r}
# You may add a "fake' `postcode` column with
# loanbook <- loanbook %>% mutate(postcode = "fake")
```
...
For example, here a non-nullable columns has `NA`'s:
...
# If you lack `postcode` we fallback to matching in the entire country instead
if (all(loanbook$postcode == "fake")) {
...
candidates_similarity %>% relocate(similarity)
```
...
The values of `suggest_match` are fist set to missing (`NA`) then set to `TRUE`
where the value of `similarity` meets all of these conditions:
...
candidates_suggest_match <- candidates_similarity %>%
  ...
  mutate(duplicates = any(duplicated(paste(company_name, postcode)))) %>%
  filter(!duplicates) %>%
  select(id, id_tilt) %>%
...
For example, this is false positive:
